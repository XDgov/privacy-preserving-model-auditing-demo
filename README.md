...
Next, edit the files `push-rpc-server.sh` and `push-proxy-server.sh` to replace the app names `test-lazovich-binary-pjc` and `test-lazovich-pjc-proxy` with the app names you would like to use. 
...
```
env RUST_LOG=info cargo run --release --bin pjc-client -- --company https://test-lazovich-pjc-proxy.app.cloud.gov --input etc/example/pjc_partner.csv --stdout --no-tls
```
